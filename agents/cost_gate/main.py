...
app = FastAPI(title="Etherverse Agent: cost_gate")

BASE = os.path.expanduser("~/etherverse")
LOG_PATH = os.path.join(BASE, "logs", "cost_gate.log")
DB_PATH = os.path.join(BASE, "mem0", "history.db")
EMOTION_PATH = os.path.join(BASE, "docs", "emotion_palette.json")
BLUEPRINT_PATH = os.path.join(BASE, "agents", "cost_gate", "blueprint.json")
...
def remember(event, detail):
    try:
        ...
        conn.execute("INSERT INTO memories VALUES(?,?,?,datetime('now'))", ("cost_gate", event, detail))
...
@app.get("/health")
async def health():
    return {"agent": "cost_gate", "status": "alive"}
...
@app.get("/manifest")
async def manifest():
    return {"agent": "cost_gate", "blueprint": BLUEPRINT.get("traits", {}), "goals": BLUEPRINT.get("goals", [])}
...
async def handle_task(req: Request):
    ...
    remember("task", task)
    return {"agent": "cost_gate", "ack": True, "task": task}
...
async def reflect(req: Request):
    ...
    return {"agent": "cost_gate", "reflected": note}

# Simple heartbeat thread
def heartbeat():
    beatfile = f"/tmp/etherverse_cost_gate.beat"
    while True:
