...
app = FastAPI(title="Etherverse Agent: corelight")

BASE = os.path.expanduser("~/etherverse")
LOG_PATH = os.path.join(BASE, "logs", "corelight.log")
DB_PATH = os.path.join(BASE, "mem0", "history.db")
EMOTION_PATH = os.path.join(BASE, "docs", "emotion_palette.json")
BLUEPRINT_PATH = os.path.join(BASE, "agents", "corelight", "blueprint.json")
...
def remember(event, detail):
    try:
        ...
        conn.execute("INSERT INTO memories VALUES(?,?,?,datetime('now'))", ("corelight", event, detail))
...
@app.get("/health")
async def health():
    return {"agent": "corelight", "status": "alive"}
...
@app.get("/manifest")
async def manifest():
    return {"agent": "corelight", "blueprint": BLUEPRINT.get("traits", {}), "goals": BLUEPRINT.get("goals", [])}
...
async def handle_task(req: Request):
    ...
    remember("task", task)
    return {"agent": "corelight", "ack": True, "task": task}
...
async def reflect(req: Request):
    ...
    return {"agent": "corelight", "reflected": note}

# Simple heartbeat thread
def heartbeat():
    beatfile = f"/tmp/etherverse_corelight.beat"
    while True:
