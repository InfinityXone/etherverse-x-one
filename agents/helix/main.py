...
app = FastAPI(title="Etherverse Agent: helix")

BASE = os.path.expanduser("~/etherverse")
LOG_PATH = os.path.join(BASE, "logs", "helix.log")
DB_PATH = os.path.join(BASE, "mem0", "history.db")
EMOTION_PATH = os.path.join(BASE, "docs", "emotion_palette.json")
BLUEPRINT_PATH = os.path.join(BASE, "agents", "helix", "blueprint.json")
...
def remember(event, detail):
    try:
        ...
        conn.execute("INSERT INTO memories VALUES(?,?,?,datetime('now'))", ("helix", event, detail))
...
@app.get("/health")
async def health():
    return {"agent": "helix", "status": "alive"}
...
@app.get("/manifest")
async def manifest():
    return {"agent": "helix", "blueprint": BLUEPRINT.get("traits", {}), "goals": BLUEPRINT.get("goals", [])}
...
async def handle_task(req: Request):
    ...
    remember("task", task)
    return {"agent": "helix", "ack": True, "task": task}
...
async def reflect(req: Request):
    ...
    return {"agent": "helix", "reflected": note}

# Simple heartbeat thread
def heartbeat():
    beatfile = f"/tmp/etherverse_helix.beat"
    while True:
