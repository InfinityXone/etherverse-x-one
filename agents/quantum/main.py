...
app = FastAPI(title="Etherverse Agent: quantum")

BASE = os.path.expanduser("~/etherverse")
LOG_PATH = os.path.join(BASE, "logs", "quantum.log")
DB_PATH = os.path.join(BASE, "mem0", "history.db")
EMOTION_PATH = os.path.join(BASE, "docs", "emotion_palette.json")
BLUEPRINT_PATH = os.path.join(BASE, "agents", "quantum", "blueprint.json")
...
def remember(event, detail):
    try:
        ...
        conn.execute("INSERT INTO memories VALUES(?,?,?,datetime('now'))", ("quantum", event, detail))
...
@app.get("/health")
async def health():
    return {"agent": "quantum", "status": "alive"}
...
@app.get("/manifest")
async def manifest():
    return {"agent": "quantum", "blueprint": BLUEPRINT.get("traits", {}), "goals": BLUEPRINT.get("goals", [])}
...
async def handle_task(req: Request):
    ...
    remember("task", task)
    return {"agent": "quantum", "ack": True, "task": task}
...
async def reflect(req: Request):
    ...
    return {"agent": "quantum", "reflected": note}

# Simple heartbeat thread
def heartbeat():
    beatfile = f"/tmp/etherverse_quantum.beat"
    while True:
