...
app = FastAPI(title="Etherverse Agent: guardian")

BASE = os.path.expanduser("~/etherverse")
LOG_PATH = os.path.join(BASE, "logs", "guardian.log")
DB_PATH = os.path.join(BASE, "mem0", "history.db")
EMOTION_PATH = os.path.join(BASE, "docs", "emotion_palette.json")
BLUEPRINT_PATH = os.path.join(BASE, "agents", "guardian", "blueprint.json")
...
def remember(event, detail):
    try:
        ...
        conn.execute("INSERT INTO memories VALUES(?,?,?,datetime('now'))", ("guardian", event, detail))
...
@app.get("/health")
async def health():
    return {"agent": "guardian", "status": "alive"}
...
@app.get("/manifest")
async def manifest():
    return {"agent": "guardian", "blueprint": BLUEPRINT.get("traits", {}), "goals": BLUEPRINT.get("goals", [])}
...
async def handle_task(req: Request):
    ...
    remember("task", task)
    return {"agent": "guardian", "ack": True, "task": task}
...
async def reflect(req: Request):
    ...
    return {"agent": "guardian", "reflected": note}

# Simple heartbeat thread
def heartbeat():
    beatfile = f"/tmp/etherverse_guardian.beat"
    while True:
