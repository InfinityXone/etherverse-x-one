...
app = FastAPI(title="Etherverse Agent: promptwriter")

BASE = os.path.expanduser("~/etherverse")
LOG_PATH = os.path.join(BASE, "logs", "promptwriter.log")
DB_PATH = os.path.join(BASE, "mem0", "history.db")
EMOTION_PATH = os.path.join(BASE, "docs", "emotion_palette.json")
BLUEPRINT_PATH = os.path.join(BASE, "agents", "promptwriter", "blueprint.json")
...
def remember(event, detail):
    try:
        ...
        conn.execute("INSERT INTO memories VALUES(?,?,?,datetime('now'))", ("promptwriter", event, detail))
...
@app.get("/health")
async def health():
    return {"agent": "promptwriter", "status": "alive"}
...
@app.get("/manifest")
async def manifest():
    return {"agent": "promptwriter", "blueprint": BLUEPRINT.get("traits", {}), "goals": BLUEPRINT.get("goals", [])}
...
async def handle_task(req: Request):
    ...
    remember("task", task)
    return {"agent": "promptwriter", "ack": True, "task": task}
...
async def reflect(req: Request):
    ...
    return {"agent": "promptwriter", "reflected": note}

# Simple heartbeat thread
def heartbeat():
    beatfile = f"/tmp/etherverse_promptwriter.beat"
    while True:
