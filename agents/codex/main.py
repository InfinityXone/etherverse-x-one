...
app = FastAPI(title="Etherverse Agent: codex")

BASE = os.path.expanduser("~/etherverse")
LOG_PATH = os.path.join(BASE, "logs", "codex.log")
DB_PATH = os.path.join(BASE, "mem0", "history.db")
EMOTION_PATH = os.path.join(BASE, "docs", "emotion_palette.json")
BLUEPRINT_PATH = os.path.join(BASE, "agents", "codex", "blueprint.json")
...
def remember(event, detail):
    try:
        ...
        conn.execute("INSERT INTO memories VALUES(?,?,?,datetime('now'))", ("codex", event, detail))
...
@app.get("/health")
async def health():
    return {"agent": "codex", "status": "alive"}
...
@app.get("/manifest")
async def manifest():
    return {"agent": "codex", "blueprint": BLUEPRINT.get("traits", {}), "goals": BLUEPRINT.get("goals", [])}
...
async def handle_task(req: Request):
    ...
    remember("task", task)
    return {"agent": "codex", "ack": True, "task": task}
...
async def reflect(req: Request):
    ...
    return {"agent": "codex", "reflected": note}

# Simple heartbeat thread
def heartbeat():
    beatfile = f"/tmp/etherverse_codex.beat"
    while True:
