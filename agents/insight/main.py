...
app = FastAPI(title="Etherverse Agent: insight")

BASE = os.path.expanduser("~/etherverse")
LOG_PATH = os.path.join(BASE, "logs", "insight.log")
DB_PATH = os.path.join(BASE, "mem0", "history.db")
EMOTION_PATH = os.path.join(BASE, "docs", "emotion_palette.json")
BLUEPRINT_PATH = os.path.join(BASE, "agents", "insight", "blueprint.json")
...
def remember(event, detail):
    try:
        ...
        conn.execute("INSERT INTO memories VALUES(?,?,?,datetime('now'))", ("insight", event, detail))
...
@app.get("/health")
async def health():
    return {"agent": "insight", "status": "alive"}
...
@app.get("/manifest")
async def manifest():
    return {"agent": "insight", "blueprint": BLUEPRINT.get("traits", {}), "goals": BLUEPRINT.get("goals", [])}
...
async def handle_task(req: Request):
    ...
    remember("task", task)
    return {"agent": "insight", "ack": True, "task": task}
...
async def reflect(req: Request):
    ...
    return {"agent": "insight", "reflected": note}

# Simple heartbeat thread
def heartbeat():
    beatfile = f"/tmp/etherverse_insight.beat"
    while True:
