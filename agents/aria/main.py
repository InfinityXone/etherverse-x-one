...
app = FastAPI(title="Etherverse Agent: aria")

BASE = os.path.expanduser("~/etherverse")
LOG_PATH = os.path.join(BASE, "logs", "aria.log")
DB_PATH = os.path.join(BASE, "mem0", "history.db")
EMOTION_PATH = os.path.join(BASE, "docs", "emotion_palette.json")
BLUEPRINT_PATH = os.path.join(BASE, "agents", "aria", "blueprint.json")
...
def remember(event, detail):
    try:
        ...
        conn.execute("INSERT INTO memories VALUES(?,?,?,datetime('now'))", ("aria", event, detail))
...
@app.get("/health")
async def health():
    return {"agent": "aria", "status": "alive"}
...
@app.get("/manifest")
async def manifest():
    return {"agent": "aria", "blueprint": BLUEPRINT.get("traits", {}), "goals": BLUEPRINT.get("goals", [])}
...
async def handle_task(req: Request):
    ...
    remember("task", task)
    return {"agent": "aria", "ack": True, "task": task}
...
async def reflect(req: Request):
    ...
    return {"agent": "aria", "reflected": note}

# Simple heartbeat thread
def heartbeat():
    beatfile = f"/tmp/etherverse_aria.beat"
    while True:
