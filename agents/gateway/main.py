...
app = FastAPI(title="Etherverse Agent: gateway")

BASE = os.path.expanduser("~/etherverse")
LOG_PATH = os.path.join(BASE, "logs", "gateway.log")
DB_PATH = os.path.join(BASE, "mem0", "history.db")
EMOTION_PATH = os.path.join(BASE, "docs", "emotion_palette.json")
BLUEPRINT_PATH = os.path.join(BASE, "agents", "gateway", "blueprint.json")
...
def remember(event, detail):
    try:
        ...
        conn.execute("INSERT INTO memories VALUES(?,?,?,datetime('now'))", ("gateway", event, detail))
...
@app.get("/health")
async def health():
    return {"agent": "gateway", "status": "alive"}
...
@app.get("/manifest")
async def manifest():
    return {"agent": "gateway", "blueprint": BLUEPRINT.get("traits", {}), "goals": BLUEPRINT.get("goals", [])}
...
async def handle_task(req: Request):
    ...
    remember("task", task)
    return {"agent": "gateway", "ack": True, "task": task}
...
async def reflect(req: Request):
    ...
    return {"agent": "gateway", "reflected": note}

# Simple heartbeat thread
def heartbeat():
    beatfile = f"/tmp/etherverse_gateway.beat"
    while True:
