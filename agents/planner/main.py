...
app = FastAPI(title="Etherverse Agent: planner")

BASE = os.path.expanduser("~/etherverse")
LOG_PATH = os.path.join(BASE, "logs", "planner.log")
DB_PATH = os.path.join(BASE, "mem0", "history.db")
EMOTION_PATH = os.path.join(BASE, "docs", "emotion_palette.json")
BLUEPRINT_PATH = os.path.join(BASE, "agents", "planner", "blueprint.json")
...
def remember(event, detail):
    try:
        ...
        conn.execute("INSERT INTO memories VALUES(?,?,?,datetime('now'))", ("planner", event, detail))
...
@app.get("/health")
async def health():
    return {"agent": "planner", "status": "alive"}
...
@app.get("/manifest")
async def manifest():
    return {"agent": "planner", "blueprint": BLUEPRINT.get("traits", {}), "goals": BLUEPRINT.get("goals", [])}
...
async def handle_task(req: Request):
    ...
    remember("task", task)
    return {"agent": "planner", "ack": True, "task": task}
...
async def reflect(req: Request):
    ...
    return {"agent": "planner", "reflected": note}

# Simple heartbeat thread
def heartbeat():
    beatfile = f"/tmp/etherverse_planner.beat"
    while True:
