...
app = FastAPI(title="Etherverse Agent: pickybot")

BASE = os.path.expanduser("~/etherverse")
LOG_PATH = os.path.join(BASE, "logs", "pickybot.log")
DB_PATH = os.path.join(BASE, "mem0", "history.db")
EMOTION_PATH = os.path.join(BASE, "docs", "emotion_palette.json")
BLUEPRINT_PATH = os.path.join(BASE, "agents", "pickybot", "blueprint.json")
...
def remember(event, detail):
    try:
        ...
        conn.execute("INSERT INTO memories VALUES(?,?,?,datetime('now'))", ("pickybot", event, detail))
...
@app.get("/health")
async def health():
    return {"agent": "pickybot", "status": "alive"}
...
@app.get("/manifest")
async def manifest():
    return {"agent": "pickybot", "blueprint": BLUEPRINT.get("traits", {}), "goals": BLUEPRINT.get("goals", [])}
...
async def handle_task(req: Request):
    ...
    remember("task", task)
    return {"agent": "pickybot", "ack": True, "task": task}
...
async def reflect(req: Request):
    ...
    return {"agent": "pickybot", "reflected": note}

# Simple heartbeat thread
def heartbeat():
    beatfile = f"/tmp/etherverse_pickybot.beat"
    while True:
