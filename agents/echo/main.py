...
app = FastAPI(title="Etherverse Agent: echo")

BASE = os.path.expanduser("~/etherverse")
LOG_PATH = os.path.join(BASE, "logs", "echo.log")
DB_PATH = os.path.join(BASE, "mem0", "history.db")
EMOTION_PATH = os.path.join(BASE, "docs", "emotion_palette.json")
BLUEPRINT_PATH = os.path.join(BASE, "agents", "echo", "blueprint.json")
...
def remember(event, detail):
    try:
        ...
        conn.execute("INSERT INTO memories VALUES(?,?,?,datetime('now'))", ("echo", event, detail))
...
@app.get("/health")
async def health():
    return {"agent": "echo", "status": "alive"}
...
@app.get("/manifest")
async def manifest():
    return {"agent": "echo", "blueprint": BLUEPRINT.get("traits", {}), "goals": BLUEPRINT.get("goals", [])}
...
async def handle_task(req: Request):
    ...
    remember("task", task)
    return {"agent": "echo", "ack": True, "task": task}
...
async def reflect(req: Request):
    ...
    return {"agent": "echo", "reflected": note}

# Simple heartbeat thread
def heartbeat():
    beatfile = f"/tmp/etherverse_echo.beat"
    while True:
