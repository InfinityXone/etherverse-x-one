#!/usr/bin/env bash
set -euo pipefail
echo "[*] Setting up the full Etherverse Dashboard and Global Journal System..."

# --- Ensure environment & venv ---
if [ ! -d "$HOME/etherverse/.venv" ]; then
  python3 -m venv "$HOME/etherverse/.venv"
fi
source "$HOME/etherverse/.venv/bin/activate"
pip install --upgrade pip flask matplotlib pandas

# --- Create dashboard directory ---
mkdir -p ~/etherverse/dashboard

# --- Write the Flask dashboard ---
cat > ~/etherverse/dashboard/app.py <<'EOF'
#!/usr/bin/env python3
"""
...
chmod +x ~/etherverse/dashboard/app.py

# --- Optional: launch the dashboard immediately ---
echo "[✓] Dashboard created. Launching on http://localhost:8080 ..."
python ~/etherverse/dashboard/app.py
